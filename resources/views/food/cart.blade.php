                ...
                <tbody>
                    @if ($cartItems->count() > 0)
                        @foreach ($cartItems as $cartItem)
                            <tr>
                                <th><img width="60" height="60" src="{{ asset('assets/img/' . $cartItem->image . '') }}"
                                        alt=""></th>
                                <td>{{ $cartItem->name }}</td>
                                <td>${{ $cartItem->price }}</td>
                                <td><a href="{{ route('food.delete.cart', $cartItem->food_id) }}"
                                        class="btn btn-danger text-white">delete</td>
                            </tr>
                        @endforeach
                    @else
                        <h3 class="alert alert-success">cart empty</h3>
                    @endif
                </tbody>
            </table>
